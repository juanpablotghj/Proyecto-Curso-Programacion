<?php
    ...
    if(isset($_REQUEST['action']) && !empty($_REQUEST['action'])){
        ...
        if($_REQUEST['action'] == 'addToCart' && !empty($_REQUEST['id'])){
            ...
            $result = $conexion->editMateriaPrima($productID);
            ...
            $result = $conexion->ConsecutivoOrden();
            ...
            $insertOrder = $conexion->CrearOrden($consecutivo,$users_id,$total_price,$created,$modified,$status);
            ...
            if($insertOrder){
                ...
                foreach($cartItems as $item){
                    ...
                    $insertOrderItems = $conexion->CrearDetOrden($consecutivo,$product_id,$quantity);
                    $detmateria = $conexion->editMateriaPrima($product_id);

                    foreach($detmateria->fetchAll(PDO::FETCH_OBJ) as $r){
                        $codigobarras = $r->codbarra;
                    }

                    $actualizarinventario = $conexion->updateInvCompra($codigobarras,$quantity);
